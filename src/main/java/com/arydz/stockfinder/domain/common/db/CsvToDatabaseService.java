...
public class CsvToDatabaseService implements FilesToDatabaseService<Long> {
    ...
    @Override
    public Long run(List<SimpleStock> simpleStockList, Path folderPath, PreparedStatement preparedStatement) {

        log.info("About to save data from {} folder", folderPath);

        try (Stream<Path> paths = Files.list(folderPath)) {

            return paths.map(filePath -> saveCSVInDatabase(simpleStockList, filePath, preparedStatement))
                    .reduce(0L, Long::sum);
    ...
    private long saveCSVInDatabase(List<SimpleStock> simpleStockList, Path filePath, PreparedStatement ps) {
        ...
        try (Reader reader = Files.newBufferedReader(filePath); CSVReader csvReader = getCsvReader(reader)) {
            ...
            Optional<Long> stockIdOptional = simpleStockList.stream()
                    .filter(s -> s.getTicker().equalsIgnoreCase(currentTicker))
                    .map(SimpleStock::getId)
                    .findAny();

            if (stockIdOptional.isPresent()) {
                Long stockId = stockIdOptional.get();
                saveInDatabase(stockId, ps, csvReader);
                count++;
            }
